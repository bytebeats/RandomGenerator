 ...
 * @Created on 2021/9/2 11:13
 * @Version 1.0
 * @Description 枚举类: 常见银行名称
 */
...
enum class BankInfo(
    ...
    val debtCardPrefixes: IntArray? = null,
    val creditCardPrefixes: IntArray? = null
) {
    /**
     * <pre>
     * 中国工商银行
     ...
     */
    ICBC(
        "102", "中国工商银行", "工行", intArrayOf(622200, 955880),
        intArrayOf(427020, 427030, 530990, 622230, 622235, 622210, 622215)
    ),
     ...
     * </pre>
     */
    CR("999999", "华润银行", null, intArrayOf(622363)),
    ...
    var allCardPrefixes: IntArray? = null

    init {
        var size = 0
        if (debtCardPrefixes?.isNotEmpty() == true) {
            size += debtCardPrefixes.size
        }
        if (creditCardPrefixes?.isNotEmpty() == true) {
            size += creditCardPrefixes.size
        }
        if (size > 0) {
            var i = 0
            allCardPrefixes = IntArray(size)
            debtCardPrefixes?.let {
                for (prefix in it) {
                    allCardPrefixes?.set(i++, prefix)
                }
            }
            creditCardPrefixes?.let {
                for (prefix in it) {
                    allCardPrefixes?.set(i++, prefix)
                }
            }
